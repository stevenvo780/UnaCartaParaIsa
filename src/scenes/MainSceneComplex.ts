...
export default class MainScene extends Phaser.Scene {
  ...
  async create() {
    ...
    try {
      ...
      this.progressManager?.startPhase(
        "world_generation",
        "Generando terreno simple 32x32...",
      );
      ...
      // 7. Usar mundo simple SIN DiverseWorldComposer para evitar hang
      this.progressManager?.startPhase(
        "world_composition",
        "Preparando mundo simple...",
      );
      logAutopoiesis.info("🎨 Usando mundo simple para evitar cuelgue...");
      
      // Crear mundo compuesto simple sin DiverseWorldComposer  
      const composedWorld = {
        layers: baseWorld.layers.map(layer => ({
        ...
        clusters: [],
        stats: {
          totalAssets: baseWorld.layers.reduce((sum, layer) => sum + (layer.assets?.length || 0), 0),
          diversityIndex: 0.5,
          clusterCount: 0,
          layerCount: baseWorld.layers.length,
          compositionTime: 0,
        }
      };
      this.progressManager?.completePhase(
        "world_composition",
        "Mundo simple preparado",
      );
  ...
  private generateOrganicWorld(): GeneratedWorld {
    const worldWidth = 32;  // Muy pequeño para testing
