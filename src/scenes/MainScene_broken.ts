...
export class MainScene extends Phaser.Scene {
  ...
  async init() {
    ...
    const initResult = await SceneInitializationManager.initialize();
    this.gameState = initResult.gameState;
    
    // Adaptar el resultado al formato esperado de GeneratedWorldData
    this.generatedWorldData = {
      zones: [], // Se llenarán después desde el biome system
      mapElements: [], // Se llenarán después desde la composición
      terrainTiles: [],
      roads: [],
      objectLayers: [],
      worldSize: { width: 2400, height: 1600 },
      generatorVersion: "DiverseWorldComposer_v1.0",
      // Guardamos los datos del nuevo sistema para uso posterior
      composedWorld: initResult.generatedWorldData.composedWorld,
      biomeSystem: initResult.generatedWorldData.biomeSystem,
    } as any;

    this.registry.set("gameState", this.gameState);
    this.registry.set("generatedWorldData", this.generatedWorldData);
  }
  ...
  async create() {
    ...
    // Temporalmente deshabilitar creación de entidades para debug
    logAutopoiesis.info("⚠️ Sistemas principales temporalmente deshabilitados para debug");
    ...
    this.time.delayedCall(2000, () => {
      logAutopoiesis.info("🎮 Game ready! Press H for help");
    });
    */

    logAutopoiesis.info("✅ MainScene básico creado (sin sistemas complejos)");
  }
  ...
  private async initializeManagers(): Promise<void> {
    // Temporalmente deshabilitar GameLogicManager para debuggear
    logAutopoiesis.info("⚠️ GameLogicManager temporalmente deshabilitado para debug");
    
    // this.gameLogicManager = new GameLogicManager(this, this.gameState);
    // this.gameLogicManager.initialize();

    // Temporalmente deshabilitar WorldRenderer para debuggear
    logAutopoiesis.info("⚠️ WorldRenderer temporalmente deshabilitado para debug");
    ...
    // Forward day/night time and weather updates to UIScene top bar and modals
    this.events.on("timeChanged", (payload: any) => {
      uiScene.events.emit("timeChanged", payload);
    });
    this.events.on("weatherChanged", (payload: any) => {
      uiScene.events.emit("weatherChanged", payload);
    });
  ...
  update() {
    ...
    if (this.worldRenderer && now - this.lastWorldUpdate > 200) {
      ...
      this.worldRenderer.updateCulling(camera.scrollX, camera.scrollY);
  ...
  public getGameStats() {
    return {
      ...
      renderer: this.worldRenderer?.getRenderedStats(),
      entities: {
