...
export class TerrainGenerator {
  ...
  private smoothBiomeTransitions(biomeMap: BiomeType[][]): BiomeType[][] {
    ...
    for (let y = 1; y < this.config.height - 1; y++) {
      for (let x = 1; x < this.config.width - 1; x++) {
        const neighbors = [
          biomeMap[y-1][x-1], biomeMap[y-1][x], biomeMap[y-1][x+1],
          biomeMap[y][x-1],   biomeMap[y][x],   biomeMap[y][x+1],
          biomeMap[y+1][x-1], biomeMap[y+1][x], biomeMap[y+1][x+1]
        ];
        ...
        for (const [biome, count] of counts) {
          if (count > maxCount && count >= 5) { // Al menos 5 de 9 neighbors
            maxCount = count;
            dominantBiome = biome;
          }
        }
  ...
  private selectTreeAsset(biomeDef: BiomeDefinition, x: number, y: number): string | null {
    const clustering = biomeDef.assets.trees.clustering;
