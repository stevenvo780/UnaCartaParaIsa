...
export class VoronoiGenerator {
  ...
  generateRegions(numRegions: number, minDistance = 100): VoronoiRegion[] {
    logAutopoiesis.info("🔸 Generando regiones Voronoi", { numRegions, minDistance });
   ...
   */
  private poissonDiskSampling(targetCount: number, minDistance: number): Point[] {
    const points: Point[] = [];
    const active: Point[] = [];
    const cellSize = minDistance / Math.sqrt(2);
    const cols = Math.ceil(this.width / cellSize);
    const rows = Math.ceil(this.height / cellSize);
    const grid: (Point | null)[][] = Array(rows).fill(null).map(() => Array(cols).fill(null));
   ...
   */
  private getVoronoiCellVertices(delaunay: Delaunator<ArrayLike<number>>, siteIndex: number): Point[] {
    ...
    for (let t = 0; t < triangles.length; t += 3) {
      if (triangles[t] === siteIndex || triangles[t + 1] === siteIndex || triangles[t + 2] === siteIndex) {
    ...
    for (const triangleIndex of triangleIndices) {
      ...
      const circumcenter = this.circumcenter(
        delaunay.coords[2 * i], delaunay.coords[2 * i + 1],
        delaunay.coords[2 * j], delaunay.coords[2 * j + 1],
        delaunay.coords[2 * k], delaunay.coords[2 * k + 1]
      );
   ...
   */
  private circumcenter(ax: number, ay: number, bx: number, by: number, cx: number, cy: number): Point {
    const dx = bx - ax;
    const dy = by - ay;
    const ex = cx - ax;
    const ey = cy - ay;
  ...
  assignBiomes(regions: VoronoiRegion[]): VoronoiRegion[] {
    const biomes = [
      "GRASSLAND", "FOREST", "DESERT", "MOUNTAINOUS", 
      "WETLAND", "COASTAL", "VILLAGE", "WASTELAND"
    ];
