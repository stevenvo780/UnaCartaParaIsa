...
export const calculateActivityPriority = (
  ...
  if (effects.optimalDuration && isFinite(timeSpentInActivity) && isFinite(effects.optimalDuration) && timeSpentInActivity > effects.optimalDuration * 1.5) {
    priority *= 0.5;
  }
...
export const applyActivityEffectsWithTimeModifiers = (
  ...
  if (activity === 'RESTING' && activityModifier) {
    ...
    if ('sleepinessRecoveryBonus' in activityModifier) {
      const sleepBonus = (effects.perMinute.sleepiness || 0) * (activityModifier.sleepinessRecoveryBonus - 1) * minutesElapsed;
