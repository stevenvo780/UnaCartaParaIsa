...
export class MemoryManager {
    ...
    public set<T>(key: string, data: T, ttl: number = 300000): boolean {
        try {
            ...
            if (this.activeOperations >= MEMORY_LIMITS.MAX_CONCURRENT_OPERATIONS) {
                logAutopoiesis.warn("Demasiadas operaciones concurrentes, rechazando cache");
                return false;
            }
   ...
   */
    public async processArrayInChunks<T, R>(
        ...
        if (array.length > MEMORY_LIMITS.MAX_ARRAYS_LENGTH) {
            ...
            throw new Error(`Array excede límite máximo de ${MEMORY_LIMITS.MAX_ARRAYS_LENGTH} elementos`);
        }
        ...
        for (let i = 0; i < array.length; i += chunkSize) {
            ...
            try {
                ...
                logAutopoiesis.error(`Error procesando chunk ${i}-${i + chunkSize}`, error);
                throw error;
            }
        }
   ...
   */
    private async waitForMemoryImprovement(maxWait: number = 5000): Promise<void> {
