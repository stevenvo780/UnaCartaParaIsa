...
export class EnhancedAssetManager extends AssetManager {
  ...
  public getAssetsByCategory(): {
    ...
    const ruins = allAssets.filter(asset => asset.key.includes('ruins')).map(asset => asset.key);

    const animals = allAssets.filter(asset => asset.key.startsWith('animal_')).map(asset => asset.key);

    const decorations = allAssets.filter(asset => asset.key.startsWith('decoration_')).map(asset => asset.key);
  ...
  public getRuinsByBiome(biomeType: string): string[] {
    ...
    return this.getAssetsByCategory().ruins.filter(ruin => ruin.includes(ruinPrefix));
  }
  ...
  public getStructuresByBiome(biomeType: string): string[] {
    ...
    return structures.filter(
      structure => structure.includes('house_basic') || structure.includes('well') || structure.includes('fence')
    );
  }
  ...
  public getAnimalsByBiome(biomeType: string): string[] {
    ...
    return animals.filter(animal => allowedAnimals.some(allowed => animal.includes(allowed)));
  }
  ...
  public getEnhancedAssetStats(): {
    ...
    const { structures, ruins, animals, decorations } = this.getAssetsByCategory();
    ...
    const byBiome = {
      grassland: this.getRuinsByBiome('grassland').length + this.getAnimalsByBiome('grassland').length,
      forest: this.getRuinsByBiome('forest').length + this.getAnimalsByBiome('forest').length,
      mystical: this.getRuinsByBiome('mystical').length + this.getAnimalsByBiome('mystical').length,
      wetland: this.getRuinsByBiome('wetland').length + this.getAnimalsByBiome('wetland').length,
      mountainous: this.getRuinsByBiome('mountainous').length + this.getAnimalsByBiome('mountainous').length,
      village: this.getStructuresByBiome('village').length + this.getAnimalsByBiome('village').length,
    };

    return {
      byCategory,
      byBiome,
      loadingSuccess: (baseStats.loaded / (baseStats.loaded + baseStats.failed)) * 100,
    };
  }
}
