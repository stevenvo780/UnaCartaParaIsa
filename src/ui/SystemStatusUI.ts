...
export class SystemStatusUI {
  ...
  private createUI(): void {
    ...
    this.container = this.scene.add.container(20, this.scene.cameras.main.height - 150);
  ...
  private initializeSystemIndicators(): void {
    const systems = [
      { name: "World", status: "active", details: "Biomas generados", color: 0x27ae60 },
      { name: "AI", status: "active", details: "Entidades pensando", color: 0x3498db },
      { name: "Needs", status: "warning", details: "Monitoreo activo", color: 0xf39c12 },
      { name: "Movement", status: "active", details: "Pathfinding OK", color: 0x2ecc71 },
      { name: "Dialogue", status: "active", details: "Cartas disponibles", color: 0x9b59b6 }
    ];
    ...
    systems.forEach((system, index) => {
      ...
      const group = this.scene.add.group([statusCircle, systemName, systemDetails]);
  ...
  public showSystemEvent(message: string, type: "info" | "warning" | "error" = "info"): void {
    const colors = {
      info: 0x3498db,
      warning: 0xf39c12, 
      error: 0xe74c3c
