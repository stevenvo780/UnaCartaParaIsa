...
export class AnimatedGameEntity extends GameEntity {
  ...
  constructor(
    ...
    logAutopoiesis.debug("🎭 AnimationManager available", {
      available: !!this.animationManager,
    });
    ...
    if (this.animationManager) {
      ...
      if (this.animationManager.hasAnimation(initialAnimation)) {
        ...
        logAutopoiesis.debug(
          `🎭 Trying fallback animation: ${fallbackAnimation}`,
        );
        logAutopoiesis.debug(
        ...
        if (
          ...
          logAutopoiesis.debug(
            `🎭 Fallback animation found: ${fallbackAnimation}`,
          );
          ...
          logAutopoiesis.debug(
            `🎭 All animations failed for ${entityId}, using static`,
          );
          logAutopoiesis.warn(
    ...
    logAutopoiesis.debug(
      `🎭 AnimatedGameEntity constructor completed for ${entityId}`,
    );
  }
