...
class AssetCleanup {
  ...
  async getAllAssets(): Promise<string[]> {
    ...
    const scanAssets = async (dir: string, baseDir: string = '') => {
      try {
        const entries = await fs.readdir(dir, { withFileTypes: true });
  ...
  async generateUsageReport(): Promise<AssetUsageReport> {
    ...
    for (const asset of allAssets) {
      const isUsed = Array.from(usedReferences).some(ref => 
   ...
   */
  async cleanupUnusedAssets(dryRun: boolean = true): Promise<void> {
